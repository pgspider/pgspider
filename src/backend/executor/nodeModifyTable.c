...
ExecModifyTable(PlanState *pstate)
{
	...
	for (;;)
	{
		...
		if (IsA(subplanstate, ForeignScanState))
			fsState = (ForeignScanState *) subplanstate;
		else if (IsA(subplanstate, ResultState))
		{
			ResultState *rsState = castNode(ResultState, subplanstate);

			fsState = (ForeignScanState *) outerPlanState(rsState);
		}
		else if (IsA(subplanstate, AppendState))
		{
			/*
			 * When there are partition tables, an Append node which has ForeignScan as inner plan will be created.
			 * Look for ForeignScanState, use appendState->as_whichplan to distinguish which plan is executing.
			 */
			AppendState *appendState = castNode(AppendState, subplanstate);

			if (appendState->as_whichplan != INVALID_SUBPLAN_INDEX)
				fsState = (ForeignScanState *) appendState->appendplans[appendState->as_whichplan];
		}

		if (fsState)
			resultRelInfo->spd_fsstate = fsState->spd_fsstate;
		else
