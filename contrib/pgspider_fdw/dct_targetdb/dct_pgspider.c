 ...
 *
 * IDENTIFICATION
 *		  contrib/pgspider_fdw/dct_pgspider/dct_pgspider.c
 *
 *-------------------------------------------------------------------------
 */
 ...
 */
typedef struct PGSpiderOptions
{
	char	*host;
	int		 port;
	char	*dbname;
...
/*
 * If data source is pgspider, pgspider_fdw can validate all the options, so there is no unique option.
 * This function always return false.
 */
bool
is_pgspider_unique_option(const char *option_name)
{
	return false;
}
 ...
 */
static void
pgspider_get_option_values(Relation rel, DataCompressionTransferOption * dct_option, PGSpiderOptions *opts)
{
	List	   *options;
	ListCell   *lc;
 ...
 */
void
pgspider_PrepareDDLRequestData(StringInfo jsonBody,
							   int mode,
							   Relation rel,
							   DDLData ddlData,
							   DataCompressionTransferOption * dct_option)
{
	PGSpiderOptions opts;

	memset(&opts, 0, sizeof(opts));
	pgspider_get_option_values(rel, dct_option, &opts);
 ...
 */
void
pgspider_PrepareInsertRequestData(StringInfo jsonBody,
